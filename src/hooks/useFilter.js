...
const reviewSchema = {
  ...
  required: ["name", "email", "rating", "comment", "productSKU"],
};
    ...
    if (file && file.type === "application/json") {
      ...
      fileReader.onload = async (event) => {
        ...
        if (location.pathname === "/categories") {
          ...
          data = json.map((value) => {
            let parentName = json?.find((item) => item?.term_id === value?.parent)?.name || "";
        ...
        if (location.pathname === "/reviews") {
          data = json.map((comment) => {
            return {
              comment_date: comment?.comment_date,

              product: comment.comment_post_ID, // Assuming comment_post_ID is the product ID
              productSKU: comment.product_SKU,  // Include the product SKU
  ...
  const handleUploadMultiple = (e) => {
    ...
    if (selectedFile.length > 1) {
      ...
      if (location.pathname === "/reviews") {
        setLoading(true);
        let attributeDataValidation = selectedFile.map((value) =>
          ajv.validate(reviewSchema, value)
        );
        ...
        if (validationData) {
          ReviewServices.addAllReviews(selectedFile)
            .then((res) => {
              setLoading(false);
              setIsUpdate(true);
              notifySuccess(res.message);
